...
export default function AiSection({ compoundFormula }) {
  ...
  const serverPort = process.env.PORT || 5000;  //port is not taken directly from env so i added 5000 manually
    ...
    <div className="overflow-y-scroll p-4 no-scrollbar w-full">
      {isLoading ? (
        ...
        <>
          ...
          {res && ( <div className="p-3 rounded shadow border border-light"> <MarkdownRenderer content={res} /> </div> )}
        </>
      )}
    </div>
  );
